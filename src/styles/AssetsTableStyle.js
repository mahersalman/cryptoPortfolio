export const AssestsTableStyle = {
    th: "px-4 py-2 text-center text-xs font-medium uppercase tracking-wider border-b border-gray-300 dark:border-gray-600",
    tableContainer: "w-full overflow-x-auto rounded-lg border border-gray-300 dark:border-gray-600 p-2 bg-gray-100 dark:bg-gray-800 text-black dark:text-white  overflow-x-auto max-h-[calc(50vh)] overflow-y-auto",
    table: "min-w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-800 text-black dark:text-white text-sm",
    thead: "bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-50 rounded-t-lg",
    tbody: "bg-gray-100 dark:bg-gray-800 text-black dark:text-white divide-y divide-gray-300 dark:divide-gray-700",


};